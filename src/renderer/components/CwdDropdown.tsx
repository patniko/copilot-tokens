  ...
  return (
    <div className="relative flex items-center" ref={ref}>
      <button
        ...
        className="flex items-center gap-1.5 font-mono text-[var(--text-primary)] hover:text-[var(--accent-gold)] transition-colors cursor-pointer truncate max-w-[300px]"
        title={cwd || 'Click to set working directory'}
      >
        <span className={!cwd ? 'text-red-500' : 'text-[var(--text-secondary)]'}>{!cwd ? '⚠️' : '📂'}</span>
        <span className="truncate">{cwd ? cwd.split('/').pop() || cwd : '(no working directory set)'}</span>
